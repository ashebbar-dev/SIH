#!/usr/bin/env python3
"""Fixed single-resampling control for the prior registration diagnostic.
...
Final ECC images are resampled directly from original capture pixels using the composed transform.
...
def compose_and_render(capture, source_shape, orb_homography, ecc_inverse_warp):
    """Compose raw→ORB and ORB←reference maps, then sample raw pixels once."""
    inverse = np.eye(3, dtype=np.float64)
    supplied = np.asarray(ecc_inverse_warp, dtype=np.float64)
    inverse[:supplied.shape[0], :] = supplied
    homography = np.linalg.inv(inverse) @ np.asarray(orb_homography, dtype=np.float64)
    result = cv2.warpPerspective(
        capture, homography, (source_shape[1], source_shape[0]),
        flags=cv2.INTER_LANCZOS4, borderMode=cv2.BORDER_CONSTANT,
        borderValue=(255, 255, 255))
    return result, homography


def composition_self_check():
    source = np.full((120, 160, 3), 255, np.uint8)
    cv2.rectangle(source, (40, 40), (80, 70), (30, 30, 30), -1)
    forward = np.array([[1, 0, 5], [0, 1, -3], [0, 0, 1]], dtype=float)
    capture = cv2.warpPerspective(source, forward, (160, 120), borderValue=(255, 255, 255))
    correction = np.array([[1, 0, 2], [0, 1, -1]], dtype=float)
    result, combined = compose_and_render(capture, source.shape, np.linalg.inv(forward), correction)
    expected = cv2.warpAffine(source, np.array([[1, 0, -2], [0, 1, 1]], dtype=float),
                              (160, 120), borderValue=(255, 255, 255))
    np.testing.assert_array_equal(result[10:-10, 10:-10], expected[10:-10, 10:-10])
    np.testing.assert_allclose(combined, [[1, 0, -7], [0, 1, 4], [0, 0, 1]])
...
def run(output):
    ...
    payload = {'schema': 'nishan.physical-registration-single-resampling/v1',
               ...
               'opencv_version': cv2.__version__, 'self_check_passed': True, 'composition_self_check_passed': True,
               'fixed_config': {'captures': list(CAPTURES), 'profiles': [p[0] for p in PROFILES],
                                'final_rendering': 'composed transform from original capture, one final resampling',
    ...
    for name in CAPTURES:
        ...
        for profile, mode in PROFILES:
            ...
            try:
                ...
                continue
            if mode is not None:
                homography = baseline_metrics.homography_suspect_to_reference
                if homography is None:
                    raise ValueError('This fixed control requires an accepted ORB homography')
                candidate, combined = compose_and_render(
                    capture, source.shape, homography, metrics['inverse_warp'])
                metrics['composed_raw_to_reference'] = combined.tolist()
