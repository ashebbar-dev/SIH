...
def marking_diagnostics_for_row(
    logical_word: np.ndarray,
    codebook: np.ndarray,
    row: int | None,
) -> tuple[dict[str, int | float] | None, str | None]:
    if row is None:
        return None, "no participating source row; marking-condition BER is undefined"
    return (
...
def _score_observation(
    ...
    marking, marking_reason = marking_diagnostics_for_row(
    ...
    return {
        **declaration,
        ...
        "negative_marking_condition_reason": marking_reason,
...
def _failure_manifest(
    *,
    error: Exception,
    output: Path,
    declared_matrix: list[dict[str, object]],
    original_config: tardos.Parameters,
    symmetric_config: tardos.Parameters,
    versions: dict[str, str],
    implementation_hashes: dict[str, str],
    source_hash: str | None,
    original_biases: np.ndarray | None,
    original_codebook: np.ndarray | None,
    symmetric_biases: np.ndarray | None,
    symmetric_codebook: np.ndarray | None,
    original_key: bytes | None,
    symmetric_key: bytes | None,
    carrier_key: bytes | None,
    wrong_carrier_key: bytes | None,
    context_uuid: str | None,
    replay_path: Path | None,
    capacities: dict[str, int] | None,
    issued: list[dict[str, object]],
) -> dict[str, object]:
    unavailable = "preparation failed before this value was available"

    def array_hash(array: np.ndarray | None, dtype: str) -> str | None:
        return None if array is None else array_commitment(array, dtype)

    def secret_hash(secret: bytes | None) -> str | None:
        return None if secret is None else _secret_commitment(secret)

    source_copy = output / "source/synthetic-source.pdf"
    replay_exists = replay_path is not None and replay_path.is_file()
    original_bias_hash = array_hash(original_biases, "<f8")
    original_codebook_hash = array_hash(original_codebook, "u1")
    symmetric_bias_hash = array_hash(symmetric_biases, "<f8")
    symmetric_codebook_hash = array_hash(symmetric_codebook, "u1")
    original_key_hash = secret_hash(original_key)
    symmetric_key_hash = secret_hash(symmetric_key)
    carrier_key_hash = secret_hash(carrier_key)
    wrong_key_hash = secret_hash(wrong_carrier_key)
    context_commitments = (
        {
    ...
    return {
        ...
        "source": {
            ...
            "sha3_256_unavailable_reason": None
            if source_hash is not None
            else unavailable,
            "run_copy_sha3_256": sha3_file(source_copy)
            if source_copy.is_file()
            else None,
            "run_copy_sha3_256_unavailable_reason": None
            if source_copy.is_file()
            else unavailable,
        },
        "versions": versions,
        "implementation_sha3_256": implementation_hashes,
        "profiles": {
        ...
        "families": {
            "original": {
                "biases_sha3_256": original_bias_hash,
                "biases_sha3_256_unavailable_reason": None
                if original_bias_hash is not None
                else unavailable,
                "codebook_sha3_256": original_codebook_hash,
                "codebook_sha3_256_unavailable_reason": None
                if original_codebook_hash is not None
                else unavailable,
                "codebook_shape": list(original_codebook.shape)
                if original_codebook is not None
                else None,
                "codebook_shape_unavailable_reason": None
                if original_codebook is not None
                else unavailable,
                "generator_key_commitment_sha3_256": original_key_hash,
                "generator_key_commitment_unavailable_reason": None
                if original_key_hash is not None
                else unavailable,
            },
            "symmetric": {
                "biases_sha3_256": symmetric_bias_hash,
                "biases_sha3_256_unavailable_reason": None
                if symmetric_bias_hash is not None
                else unavailable,
                "codebook_sha3_256": symmetric_codebook_hash,
                "codebook_sha3_256_unavailable_reason": None
                if symmetric_codebook_hash is not None
                else unavailable,
                "codebook_shape": list(symmetric_codebook.shape)
                if symmetric_codebook is not None
                else None,
                "codebook_shape_unavailable_reason": None
                if symmetric_codebook is not None
                else unavailable,
                "generator_key_commitment_sha3_256": symmetric_key_hash,
                "generator_key_commitment_unavailable_reason": None
                if symmetric_key_hash is not None
                else unavailable,
                "same_codebook_for_both_symmetric_profiles": True,
            },
        },
        "carrier": {
            "key_commitment_sha3_256": carrier_key_hash,
            "key_commitment_unavailable_reason": None
            if carrier_key_hash is not None
            else unavailable,
            "wrong_key_commitment_sha3_256": wrong_key_hash,
            "wrong_key_commitment_unavailable_reason": None
            if wrong_key_hash is not None
            else unavailable,
            "profile_context_commitments_sha3_256": context_commitments,
            "profile_context_commitments_unavailable_reason": None
            if context_commitments is not None
            else unavailable,
            "profile_contexts_are_distinct": True,
            "dpi": DPI,
            "strength": STRENGTH,
            "rendered_capacities": capacities,
            "rendered_capacities_unavailable_reason": None
            if capacities is not None
            else unavailable,
        },
        "private_replay": {
            "path": "private/replay-state.npz",
            "sha3_256": sha3_file(replay_path) if replay_exists else None,
            "sha3_256_unavailable_reason": None if replay_exists else unavailable,
            "mode": "0600" if replay_exists else None,
            "mode_unavailable_reason": None if replay_exists else unavailable,
        },
        "issued_pdfs_completed_before_failure": issued,
        "declared_matrix": declared_matrix,
        "declared_observation_count": 57,
        "manifest_provenance_limitation": (
            "This local failure manifest is not independently signed provenance."
        ),
    }
...
def run(output: Path) -> dict[str, object]:
    ...
    source_hash = sha3_file(SOURCE) if SOURCE.is_file() else None
    original_biases: np.ndarray | None = None
    original_codebook: np.ndarray | None = None
    symmetric_biases: np.ndarray | None = None
    symmetric_codebook: np.ndarray | None = None
    original_key: bytes | None = None
    symmetric_key: bytes | None = None
    carrier_key: bytes | None = None
    wrong_carrier_key: bytes | None = None
    ...
    try:
        ...
        try:
            ...
            if not manifest_path.exists():
                _json_write(
                    ...
                    _failure_manifest(
                        ...
                        symmetric_config=symmetric_config,
                        versions=versions,
                        implementation_hashes=implementation_hashes,
                        source_hash=source_hash,
                        original_biases=original_biases,
                        original_codebook=original_codebook,
                        symmetric_biases=symmetric_biases,
                        symmetric_codebook=symmetric_codebook,
                        original_key=original_key,
                        symmetric_key=symmetric_key,
                        carrier_key=carrier_key,
                        wrong_carrier_key=wrong_carrier_key,
            ...
            # Recording must never replace the original experimental failure.
            pass
        raise
